...
export default function PageDetailNoteWrapper({ toggleLocale, toggleTheme, logout }) {
  const { id } = useParams();
...
PageDetailNoteWrapper.propTypes = {
          ...
          <div className="page-detail-wrapper">
            ...
            <div>
              ...
              <Link to="/" className="anchor-back-wrapper">
                <MdArrowBack size={30} color={theme === 'light' ? '000' : 'fff'} />
                <Text type="text-anchor-back">{locale === 'en' ? 'Back' : 'Kembali'}</Text>
              </Link>
              {Object.keys(this.state.notes).length > 0 && (
            ...
            <ButtonActionRoundedWrapper>
              <ButtonActionRounded
                type={this.state.notes?.archived ? 'archived-note' : 'archive-note'}
