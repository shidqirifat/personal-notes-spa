...
import { generateQuotes, getNavigationsLink, templateTextEmpty } from '../utils/data';
...
class PageListNote extends Component {
  ...
  async handleGetNotes() {
    ...
    const { data } =
      this.props.pageActive === 'active' ? await getActiveNotes() : await getArchivedNotes();
    this.setState((prevState) => ({
      ...prevState,
      notes: data,
      isLoading: false
  ...
  render() {
    const renderTextEmpty = () => {
      ...
      if (!keyword && pageActive === 'archived') return templateTextEmpty[locale].empty_archived;
      if (!keyword && pageActive === 'active') return templateTextEmpty[locale].empty_active;
