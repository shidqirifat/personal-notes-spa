...
export default class NewNote extends Component {
  ...
  onInputEventHandler(event) {
    ...
    this.setState((prevState) => ({
      ...
      chartLeft:
        event.target.name === 'title' ? 50 - event.target.value.length : prevState.chartLeft
    }));
  }
          ...
          <>
            ...
            <div className="new-note-wrapper">
              <Text type="title-section" style={{ textAlign: 'center', marginTop: 0 }}>
                {locale === 'en' ? 'Add New Note' : 'Tambah Catatan'}
              </Text>
              <Text
                type="paragraph"
                style={{
                ...
                {locale === 'en' ? 'Characters left' : 'Sisa karakter'}: {this.state.chartLeft}
              </Text>
              <FormNewNote
